...
var vLogin = new Vue({
	...
	data: function() {
		return {
			...
			loginError: '',
			phoneError: '', //错误提示信息
			imgSrc: METHOD_URL + firstPath + "/loginCode/findVerifyCode.sose", //图形验证码
...
function checkPhone(rule, value, callback) {
	var result = VERIFY_PHONE.test(value);
	if(!value) {
...
function comparePicCode() {
	...
	console.log(data);
	//调用业务端方法
	var url = firstPath + "/loginCode/comparePicCode";
...
function accountLogin() {
	...
	console.log(data);
	//调用业务端方法
	var url = firstPath + "/accountLogin";
...
function sendMessage() {
	if(vLogin.sendFlag) {
		...
		//调动业务端方法
		var url = firstPath + "/loginPhone/sendMessage";
...
function sendMessageSus(data) {
	...
	if(data.info == "error" || data.info == "fail") {
		vLogin.loginError = '短信发送异失败，请稍后再试！';
		return;
	}
	if(data.info == "1016") {
...
function refreshAccountImg() {
	var url = METHOD_URL + firstPath + "/loginCode/findVerifyCode.sose?number=" + Math.random();
...
function openForgetPwd() {
	//	var url = PROJECT_NAME + "/reset_password";
	var url = "./forgetPwd.html";
	window.open(url);
}